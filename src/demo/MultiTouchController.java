	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		for (int i = 0; i < histLen; i++) {
			if (secondPointerIndex >= 0)
				decodeTouchEvent(event.getHistoricalX(i),
						event.getHistoricalY(i), event.getPressure(i),
						event.getPointerCount(),
						event.getHistoricalX(secondPointerIndex, i),
						event.getHistoricalY(secondPointerIndex, i),
						event.getHistoricalPressure(secondPointerIndex, i),
				...
				decodeTouchEvent(event.getHistoricalX(i),
						event.getHistoricalY(i), event.getPressure(i),
						event.getPointerCount(), 0, 0, 0.0f,
						MotionEvent.ACTION_MOVE, true,
						event.getHistoricalEventTime(i));
		}
		...
		if (secondPointerIndex >= 0)
			decodeTouchEvent(
					...
					event.getPressure(secondPointerIndex),
					event.getAction(),
